...
def mom4_jax(keys, triu_indices, bm: Array, hh_in: Optional[Array], kk_in: Optional[Array]):
    bm_dim = bm.shape[-1]
    ...
    sigma = sigma + (144/28) * (squared_kk[triu_indices[0]] + squared_kk[triu_indices[1]])
    sigma = jnp.sqrt(sigma)

    bm_i = bm[triu_indices[0]]
    bm_j = bm[triu_indices[1]]
    ...
    levy_out = hh_i * bm_j - bm_i * hh_j + 12. * (kk_i * hh_j - hh_i * kk_j) + tilde_a

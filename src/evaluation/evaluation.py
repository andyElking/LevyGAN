...
import torch
import numpy as np
from src.model.Generator import PairNetGenerator, Generator
...
def compute_grad_norm(generator, discriminator, sample_data, config, device):
    ...
    gradient_penalty, gradient_norm = _gradient_penalty(unfixed_data, fake_data, gp_weight=0,
...
def compute_fixed_losses(generator, config, input_true_data: torch.Tensor,
    ...
    if input_true_data.shape[1] != levy_dim:
        print(f"Warning in compute_fixed_losses: input_true_bm.shape={input_true_bm.shape}, but "
    ...
    st_dev_fixed_increment = np.diag(levy_conditional_st_dev(bm[1, :].to('cpu').tolist()))

    # run generator

    fake_data = generator.sample_fake_data(bm)
    levy_fake = fake_data[:, bm_dim: (bm_dim + levy_dim)]
    marginal_w2_loss = [sqrt(ot.wasserstein_1d(levy_true[:, i], levy_fake[:, i], p=2).item()) for i in range(levy_dim)]
    ...
    if comp_joint_error and (bm_dim >= 3):
        ...
        joint_w2_loss = joint_wass_dist(levy_true[:joint_wass_dist_bsz],
                                        levy_fake[:joint_wass_dist_bsz])
    else:
        joint_w2_loss = float('inf')

    if true_fourth_moments is not None:
        mom4_errs = fourth_moment_errors(input_samples=levy_fake, true_moments=true_fourth_moments).item()
...
def avg_st_dev_error(st_dev_fixed_increment, _levy_generated):
    difference = np.abs(st_dev_fixed_increment - np.sqrt(np.abs(empirical_second_moments(_levy_generated))))
    return difference.mean()


def make_report(tester, epoch: int = None, iters: int = None, add_line_break=True, short=False):
    """
...
def draw_error_graphs(graph_filename, marginal_w2_losses_through_training, single_coord_labels,
                      discriminative_losses_through_training,
                      fourth_moments_through_training=None, joint_labels=None, descriptor: str = '',
                      iters=None):
    """
    ...
    if (fourth_moments_through_training is not None) and len(fourth_moments_through_training) > 0:
    ...
    elif (discriminative_losses_through_training is not None) and len(discriminative_losses_through_training) > 0:
...
def check_perm_inv_densities(gen: Generator, h_in: list[float] = None, only_draw_first_dim = False):
    if h_in is None:
    ...
    errors = [sqrt(ot.wasserstein_1d(bb1[:, i], bb2_prime[:, i], p=2)) for i in range(levy_dim)]
    ...
    if only_draw_first_dim:
        fig, ax = plt.subplots(nrows=1, ncols=1, sharex='all', sharey='all', figsize=(5, 3))
        ax.set_title("b(0,1)")
        labels = ['original', '1st and 2nd dim swapped']
        ax.hist((distns1[0], distns2[0]), bins=400, density=True, histtype='step', stacked=False, label=labels)
        ax.set_xlim(-1.5, 1.5)
        ax.legend(prop={'size': 6})
        fig.show()
        return

    fig, axs = plt.subplots(nrows=2, ncols=3, sharex='all', sharey='all', figsize=(16, 5))
    ...
    for i in range(6):
        ...
        labels = ['original', '1st and 2nd dim swapped']
        ax.hist((distns1[i], distns2[i]), bins=400, density=True, histtype='step', stacked=False, label=labels)
        ax.set_xlim(-1.5, 1.5)
        ax.legend(prop={'size': 6})
...
def draw_density_graphs(data_in: np.ndarray, use_levy_dim_titles=False, xlim: float = 1.5):
    """
    ...
    print(ncols)
    fig, axs = plt.subplots(nrows=nrows, ncols=ncols, sharex='all', sharey='all', figsize=(5 * ncols, 3 * nrows))
...
def draw_generator_densities(gen: Generator, generate_bb=False, data_in: torch.Tensor = None,
                             data_in_row: list[float] = None, bsz: int = 65536, xlim: float = 1.5):
    """
    ...
    if data_in_row is not None and data is None:
        data: torch.Tensor = torch.tensor([data_in_row], dtype=torch.float, device=gen.device)
...
def compute_objectives(levG, gen_conf: dict, tr_conf: dict, num_trials: int = 5, use_saves = False):
    ...
    if use_saves:
        ...
        files = [path+f for f in listdir(path) if isfile(join(path, f)) and ('best__scr' in f or 'best_4mom' in f)]
        print(len(files))
    ...
    for trial in range(len(files)):
        ...
        print(f"trial: {trial}, elapsed: {make_pretty(elapsed)}, marginal_err: {make_pretty(err)},"
    ...
    return min_marginal_loss, marginal_err, min_mom4_loss, mom4_err, min_eval_time, time_err


def full_evaluation(x_real, generator, conf, device, foster=False, gamma=0.5, num_iter=5, real_fourth_moments = None):
    """
    ...
    with torch.no_grad():
        ...
        for seed in range(num_iter):
            ...
            marginal_w2_loss = np.mean(
                np.array([100 * sqrt(ot.wasserstein_1d(levy_real[:, i], levy_fake[:, i], p=2)) for i in range(levy_dim)]))
            fourth_moment_loss = fourth_moment_errors(levy_fake, real_fourth_moments).item()
        ...
        if self.sub_matrix_dim is None:
            ...
            for row_num in range(row_iters):
                for col_num in range(col_iters):
                    X1 = X[row_num * self.sub_matrix_dim: (row_num + 1) * self.sub_matrix_dim]
                    Y1 = Y[col_num * self.sub_matrix_dim: (col_num + 1) * self.sub_matrix_dim]

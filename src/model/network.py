...
class PairNetBatchNorm(nn.Module):
    def __init__(self, num_fts):
        ...
        self.gamma = nn.Parameter(torch.ones((1, 1, num_fts), dtype=torch.float), requires_grad=True)
        self.beta = nn.Parameter(torch.zeros((1, 1, num_fts), dtype=torch.float), requires_grad=True)
    ...
    def forward(self, x, levy_dim):
        ...
        batch_var_inv = torch.pow(torch.var(x_expanded, dim=0, keepdim=True) + self.eps, exponent=-1)
        x_normalised = (x_expanded - batch_avg) * (batch_var_inv * self.gamma) + self.beta
        return x_normalised.view(bsz * levy_dim, self.num_fts).contiguous()


class LevyGANLayer(nn.Module):
    def __init__(self, layer_indim, layer_outdim, use_batch_norm, act_type, use_pairnet_bn):
        ...
        if use_batch_norm:
            self.batch_norm = PairNetBatchNorm(layer_outdim) if use_pairnet_bn else nn.BatchNorm1d(layer_outdim)
...
def create_net(gen_conf: dict, in_dim: int, out_dim: int):
    ...
    activations = gen_conf['activation'] if 'activation' in gen_conf else [('leaky', 0.2)]*(num_layers-1)
   ...
   # ===== Describe the neural net =======
    ...
    net_description = f"{num_layers}LAY_{hidden_dim}HID_{act_desc}ACT{'_BN' if batch_norm else ''}"
    ...
    def get_layer(layer_num):
        ...
        layer = LevyGANLayer(layer_indim, layer_outdim, batch_norm, activations[layer_num], pairnet_bn)
        return layer

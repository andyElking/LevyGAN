...
from src.model.Generator import Generator, get_generator, PairNetGenerator
...
training_config = {
        ...
        'chen_penalty_alpha': 1.,  # The coefficient for chen training (only relevant for Rotational_inv_LevyGAN)
...
small_table = [[['-LKY -NSZ L1', '-LKY +NSZ L1']],
               [['+LKY -NSZ L1', '+LKY +NSZ L1']]]

small_table = [[['LKY=0.01 NSZ=4 alpha=0.1', 'LKY=0.01 NSZ=4 alpha=1'], ['LKY=0.01 NSZ=8 alpha=0.1', 'LKY=0.01 NSZ=8 alpha=1']],
                [['LKY=0.2 NSZ=4 alpha=0.1', 'LKY=0.2 NSZ=4 alpha=1'], ['LKY=0.2 NSZ=8 alpha=0.1', 'LKY=0.2 NSZ=8 alpha=1']]]
...
def get_small_table(dis_batch, lie_degree):
    table = [
        [[get_result(dis_batch, lie_degree, leaky_slope, noise_size, alpha) for alpha in [1.]] for
         noise_size in [4, 8]] for leaky_slope in [0.01, 0.2]]
...
contents_to_save = f"Legend for small teble entries:\n\n" \
                   f"{legend} \n\nRESULTS:\n\n" \
                   f"{full_results}"

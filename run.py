...
def model_training(gan_config = None, gen_config = None, training_config = None, discr_config = None, tester_config = None):
    """
    ...
    # Name this run
    descriptor = f"seed_{random_seed}_{gen_config['noise_size']}" \
...
def model_evaluation(generator_dir, gen_config):
    ...
    x_real = torch.tensor(np.genfromtxt(f"samples/samples_4-dim.csv", dtype=float, delimiter=',')).to(
        dtype=torch.float, device=device)
    # Run the evaluation
    loss_dict = full_evaluation(x_real, generator, gen_config, device, real_fourth_moments=four_mom)
